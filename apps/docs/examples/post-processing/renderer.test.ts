...
const mocks=vi.hoisted(()=>({init:vi.fn()}));vi.mock('vgpu', () => ({ init: mocks.init, ...vgpuFns, prepare: (gpu: any, ...args: any[]) => (gpu?.fns?.prepare ? gpu.fns.prepare(...args) : Promise.resolve([])), clock: (gpu: any) => gpu.clock ?? { time: 0, deltaTime: 0, frameCount: 0, advance() {} } }));
