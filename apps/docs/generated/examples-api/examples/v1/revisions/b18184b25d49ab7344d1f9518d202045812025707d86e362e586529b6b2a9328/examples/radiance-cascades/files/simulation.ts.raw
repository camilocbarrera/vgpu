...
import { effect, frame, prepare, sampler, target } from "vgpu";
 ...
 * Compiles every pipeline against the formats it will actually render into.
 *
 * Dynamic multi-pass (T04-19): `jfaSteps` and `cascade` are generated by loops over
 * `scene.jumps` / `scene.cascadeCount`, so their requests are generated by iterating THOSE SAME
 * arrays — change the cascade count and this list follows. Flattening them into N literal lines
 * would have frozen a count the scene decides at construction.
 *
 * The requests keep signature literals (`{ colors: [HDR_FORMAT] }`) rather than naming a Target,
 * and that is deliberate, not a leftover: every JFA step ping-pongs between `jfa[0]`/`jfa[1]` and
 * every cascade level recycles `cascades[0]`/`cascades[1]`, so no single Target object is "the"
 * destination of a given effect. One format is what they genuinely share, and a signature is
 * exactly the thing the pipeline key is built from.
 */
export async function prepareScene(scene: RadianceScene, outputFormat: GPUTextureFormat): Promise<void> {
  await prepare(scene.gpu, [
    { draw: scene.effects.paint, target: { colors: [HDR_FORMAT] } },
    { draw: scene.effects.jfaInit, target: { colors: [SEED_FORMAT] } },
    ...scene.effects.jfaSteps.map((shader) => ({ draw: shader, target: { colors: [SEED_FORMAT] } })),
    { draw: scene.effects.sdfFinalize, target: { colors: [HDR_FORMAT] } },
    ...scene.effects.cascade.map((shader) => ({ draw: shader, target: { colors: [HDR_FORMAT] } })),
    { draw: scene.effects.present, target: { colors: [outputFormat] } },
  ]);
}
