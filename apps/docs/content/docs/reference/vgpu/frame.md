---
title: "Frame"
description: "`frame()` is both a callable one-frame submit helper and a `FrameRunner`. It borrows **one** command encoder, lets you encode render passes, compute dispatches, buffer copies and raw encoder commands in program order, then submits **exactly once**. `frame(gpu, cb)` per tick is the primitive; `frameLoop` is convenience sugar for self-contained demos — most applications delegate the loop to an animation engine and call `frame()` per tick."
---
