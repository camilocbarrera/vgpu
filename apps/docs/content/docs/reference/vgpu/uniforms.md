...
description: "One shared uniform resource created by `uniform(gpu, values)`, bindable into many pipelines at once. It adopts the binary WGSL layout lazily from the first compatible shader binding, reuses one stable buffer across shaders, and updates every pipeline bound to it with a **single** write."
---
